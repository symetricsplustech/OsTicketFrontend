...
for (const line of markdown.split(/\r?\n/)) {
  ...
  if (heading) {
    ...
    currentModule = number >= 1 && number <= 12 ? number : null;
    if (currentModule) {
...
const modules = [...permissions.entries()].map(([number, keys]) => {
  ...
  return {
    number,
    key: namespace.replace("itsm.", ""),
    label: moduleNames.get(number),
    namespace: `${namespace}.*`,
    permissions: unique,
  };
});
...
console.log(
  `Generated ${modules.reduce((sum, module) => sum + module.permissions.length, 0)} permissions across ${modules.length} modules.`,
);
