...
import {
  approvalEngineApi,
  ApprovalDashboard as DashboardType,
} from "../../services/approvals/approvalEngineApi";
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card
          label="Definitions"
          value={data.totalDefinitions}
          sub={`${data.activeDefinitions} active`}
        />
      ...
      <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            {Object.entries(data.statusCounts || {}).map(([status, count]) => (
              <div key={status} className="flex items-center justify-between">
                <span
                  className={`px-2 py-1 rounded text-xs ${statusColors[status] || "bg-gray-100 text-gray-700"}`}
                >
                  {status}
                </span>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="space-y-2">
            {Object.entries(data.entityTypeBreakdown || {}).map(
              ([type, count]) => (
