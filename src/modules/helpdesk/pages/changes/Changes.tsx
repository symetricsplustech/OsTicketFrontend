...
export default function Changes() {
  ...
  const checkConflicts = async () => {
    if (!canView)
      return toast.error(
        "You do not have permission to view change conflicts.",
      );
    ...
    try {
      const res = await api.get("/enterprise/changes/conflicts", {
        params: { start: form.windowStart, end: form.windowEnd },
      });
      setConflicts(res.data.conflicts);
      const n =
        (res.data.conflicts?.overlapping?.length || 0) +
        (res.data.conflicts?.blackouts?.length || 0);
      toast(n ? `⚠️ ${n} conflict(s) found` : "No conflicts in this window", {
        icon: n ? "⚠️" : "✅",
      });
    } catch {
    ...
    try {
      ...
      setLoadError(
        error?.response?.status === 403
          ? "You do not have permission to view changes."
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canCreate)
      return toast.error("You do not have permission to create changes.");
    setSaving(true);
    try {
      ...
      toast.success(
        n
          ? `Change raised (risk ${res.data?.change?.riskScore ?? "—"}) — ${n} conflict(s)!`
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setForm({ ...form, rollbackPlan: e.target.value })
                }
                rows={3}
                className="mt-1 input-field"
              ...
              {conflicts && (
                <div className="mt-2 text-xs bg-gray-50 border rounded-lg p-3 space-y-1">
                  {(conflicts.overlapping || []).map((c: any) => (
                    <p key={c.id}>
                      ⚠️ Overlaps <b>{c.number}</b> — {c.title} ({c.status})
                    </p>
                  ))}
                  ...
                  {(conflicts.sharedAssets || []).map((s: any, i: number) => (
                    <p key={i}>
                      🔗 Shared assets with <b>{s.number}</b>
                    </p>
                  ))}
                  {!conflicts.overlapping?.length &&
                    !conflicts.blackouts?.length &&
                    !conflicts.sharedAssets?.length && (
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={saving} className="btn-primary">
                {saving ? "Creating..." : "Submit Change Request"}
              </button>
              <button
                type="button"
