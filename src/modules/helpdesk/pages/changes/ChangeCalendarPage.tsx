      ...
      try {
        ...
        setChangeRows(
          (Array.isArray(calData)
            ? calData
            : calData?.rows || calData?.events || []) as ChangeCalendarRow[],
        );
      } catch {
        setChangeRows([]);
      }
  ...
  const eventsForDay = (day: number): CalEvent[] => {
    ...
    return events.filter((ev) => {
      ...
      const inMonth = (d: Date) =>
        d.getFullYear() === year && d.getMonth() === mon - 1;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => shiftMonth(-1)} className="btn-secondary">
            ←
          </button>
          <span className="font-semibold text-gray-900 min-w-[10rem] text-center">
            {monthLabel}
          </span>
          <button onClick={() => shiftMonth(1)} className="btn-secondary">
            →
      ...
      <div className="card overflow-hidden">
        {loading ? (
          <p className="px-6 py-12 text-center text-sm text-gray-400 animate-pulse">
            Loading calendar…
          </p>
        ) : (
          <>
