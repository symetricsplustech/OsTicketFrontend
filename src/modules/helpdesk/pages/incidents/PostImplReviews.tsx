...
const emptyQuestion = (): PirQuestion => ({
  question: "",
  answer: "",
  category: "timeline",
});
...
export default function PostImplReviews() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const data = res.data;
        setChanges(
          (Array.isArray(data) ? data : data?.changes || []) as ChangeRow[],
        );
      } catch {
        setChanges([]);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = res.data;
      setPirs(
        (Array.isArray(data) ? data : data?.rows || data?.pirs || []) as PIR[],
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="card p-6">
          <h2 className="font-semibold mb-4">
            {editingId ? "Edit PIR" : "New Post-Implementation Review"}
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  value={form.changeId}
                  onChange={(e) =>
                    setForm({ ...form, changeId: e.target.value })
                  }
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, riskRating: e.target.value })
                  }
                  className="mt-1 input-field"
                >
                  {RISK_RATINGS.map((r) => (
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {saving ? "Saving…" : editingId ? "Update PIR" : "Save PIR"}
              </button>
              <button
                type="button"
                onClick={() => {
      ...
      {loading ? (
        <p className="card px-6 py-12 text-center text-sm text-gray-400 animate-pulse">
          Loading reviews…
        </p>
      ) : pirs.length === 0 ? (
        <p className="card px-6 py-12 text-center text-sm text-gray-500">
          No post-implementation reviews yet.
        </p>
      ) : (
          ...
          {pirs.map((p) => (
            <div key={p._id} className="card p-5 flex flex-col gap-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <span
                  className={`shrink-0 px-2 py-0.5 text-xs rounded-full capitalize ${OUTCOME_BADGES[p.outcome || ""] || "bg-gray-100 text-gray-700"}`}
                >
