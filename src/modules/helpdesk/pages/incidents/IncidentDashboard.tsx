    ...
    const load = async () => {
      try {
        ...
        setStats({
          ...
          open: items.filter((i: Incident) =>
            ["new", "in_progress"].includes(i.status),
          ).length,
          onHold: items.filter((i: Incident) => i.status.startsWith("on_hold_"))
            .length,
          resolved: items.filter((i: Incident) => i.status === "resolved")
            .length,
          ...
          {recentIncidents.map((inc) => (
            ...
            >
              ...
              <div className="flex gap-2">
                <span
                  className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[inc.status] || "bg-gray-100"}`}
                >
                  {inc.status}
                </span>
