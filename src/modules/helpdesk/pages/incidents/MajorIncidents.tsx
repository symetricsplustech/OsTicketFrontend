...
export default function MajorIncidents() {
    ...
    try {
      ...
      setLoadError(
        error?.response?.status === 403
          ? "You do not have permission to view major incidents."
          : "Unable to load major incidents. Please retry.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const declare = async (e: React.FormEvent) => {
    ...
    if (!canCreate || !canUpdate)
      return toast.error(
        "You need create and update permission to declare a major incident.",
      );
    setSaving(true);
    try {
      ...
      await api.post(
        `/enterprise/incidents/${created.data.incident._id}/major`,
        { reason: form.reason },
      );
  ...
  const setStatus = async (id: string, status: string) => {
    if (!canUpdate)
      return toast.error(
        "You do not have permission to update major incidents.",
      );
    try {
      await api.put(`/enterprise/incidents/${id}`, {
        status,
        ...(status === "resolved"
  ...
  const saveCommPlan = async (id: string) => {
    if (!canUpdate)
      return toast.error(
        "You do not have permission to configure major-incident communications.",
      );
    try {
      const body = commPlan[id] || {
        cadenceMinutes: 30,
        audience: ["internal", "customer"],
      };
      const res = await api.put(
        `/enterprise/incidents/${id}/communication-plan`,
        body,
      );
  ...
  const fetchExecReport = async () => {
    try {
      const res = await api.get("/gaps2/reports/major-incidents-exec", {
        responseType: "text",
      });
      setExecReport(
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Declaration, command, comms cadence &amp; executive reporting
          </p>
        </div>
        <div className="flex gap-2">
          <Link to="/warroom" className="btn-secondary text-sm">
            Open War Room
          </Link>
          <Link to="/outages" className="btn-secondary text-sm">
            Outage Tracking
          </Link>
          {canCreate && canUpdate && (
            <button
      ...
      {showForm && (
        <form onSubmit={declare} className="card p-6 space-y-4">
          ...
          <textarea
            ...
            rows={3}
            placeholder="Service + business impact…"
            className="input-field w-full"
          />
          <textarea
            required
            ...
            rows={2}
            placeholder="Why this meets major-incident criteria *"
            className="input-field w-full"
          />
          <div className="flex gap-2">
            <button
            ...
            {loading ? (
              ...
              <tr>
                <td
                  ...
                  className="px-5 py-10 text-center text-gray-400"
                >
                  No major incidents — declare one to start command mode
                </td>
              </tr>
            ) : (
              items.map((m) => (
                <tr key={m._id} className="hover:bg-gray-50 align-top">
                  ...
                  <td className="px-5 py-3">
                    <StatusBadge
                      status={m.severity || m.priority || "critical"}
                    />
                  </td>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setCommPlan((p) => ({
                              ...
                              [m._id]: {
                                ...(p[m._id] || {}),
                                cadenceMinutes: Number(e.target.value),
                              },
                            }))
                          }
                          className="input-field text-xs w-16 py-1"
                          title="Update cadence (minutes)"
                        />
                      )}
                      {canUpdate && (
                        <button
                          onClick={() => saveCommPlan(m._id)}
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={fetchExecReport} className="btn-secondary text-xs">
            Generate summary
          </button>
        </div>
        {execReport ? (
          <pre className="text-xs bg-gray-50 border rounded-lg p-4 whitespace-pre-wrap max-h-80 overflow-y-auto">
            {execReport}
          </pre>
        ) : (
          <p className="text-xs text-gray-400">
            Generates a markdown brief of major incidents + outages for
            leadership.
          </p>
        )}
      </div>
