...
export default function OutageTracking() {
  ...
  const load = async () => {
    try {
      const { data } = await api.get("/extra/outages");
      setOutages(data);
    } catch {}
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post("/extra/outages", form);
      setShowForm(false);
  ...
  const handleTimeline = async (id: string) => {
    ...
    try {
      await api.post(`/extra/outages/${id}/timeline`, {
      ...
      load();
      const { data } = await api.get("/extra/outages");
      setSelected(data.find((o: Outage) => o._id === id) || null);
    } catch {}
  };

  const handleUpdate = async (id: string, status: string) => {
    try {
      await api.put(`/extra/outages/${id}`, {
        status,
        resolvedAt:
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <AlertTriangle className="h-6 w-6" /> Service Outage Tracking
        </h1>
        <button
      ...
      <div className="flex gap-6">
        <div className="flex-1 space-y-3">
          {outages.map((o) => (
            <div
              key={o._id}
              className={`bg-white p-4 rounded-lg border cursor-pointer hover:shadow-md transition-shadow ${selected?._id === o._id ? "ring-2 ring-blue-500" : ""}`}
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2">
                  <span
                    className={`px-2 py-1 text-xs rounded-full ${sevColor(o.severity)}`}
                  >
                    {o.severity}
                  </span>
                  <span
                    className={`px-2 py-1 text-xs rounded-full ${statusColor(o.status)}`}
                  >
                    {o.status}
                  </span>
                </div>
          ...
          <div className="w-96 bg-white rounded-lg border p-4 space-y-4">
            ...
            <div className="space-y-2 text-sm">
              <div>
                ...
                <span
                  className={`px-2 py-1 text-xs rounded-full ${statusColor(selected.status)}`}
                >
                  {selected.status}
                </span>
              </div>
            ...
            <div className="flex gap-1 flex-wrap">
              {[
                ...
                <button
                  ...
                  className={`text-xs px-2 py-1 rounded ${selected.status === s ? "bg-blue-600 text-white" : "bg-gray-100 hover:bg-gray-200"}`}
                >
                  {s}
                </button>
              ))}
            ...
            <div className="border-t pt-3">
                ...
                {selected.timeline
                  ...
                  .map((t, i) => (
                    <div key={i} className="flex items-start gap-2 text-xs">
                      ...
                      <div>
                        <span
                          className={`px-1 rounded ${statusColor(t.status)}`}
                        >
                          {t.status}
                        </span>{" "}
                        {t.message}{" "}
              ...
              <div className="flex gap-2 mt-2">
                ...
                />
                <button
                  onClick={() => handleTimeline(selected._id)}
