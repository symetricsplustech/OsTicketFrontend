...
export default function PriorityMatrixEditor() {
  ...
  const [setPriorityMatrix, { isLoading: saving }] =
    useSetPriorityMatrixMutation();
  const [computePriority, { isLoading: computing }] =
    useComputePriorityMutation();
  ...
  useEffect(() => {
    if (Array.isArray(matrix)) {
      ...
      for (const i of IMPACTS)
        for (const u of URGENCIES) next[key(i, u)] = "medium";
      (matrix as any[]).forEach((c) => {
        if (c?.impact && c?.urgency)
          next[key(c.impact, c.urgency)] = c.priority || "medium";
      });
      setCells(next);
    }
  }, [matrix]);
      ...
      await setPriorityMatrix({
        cells: IMPACTS.flatMap((impact) =>
          URGENCIES.map((urgency) => ({
            impact,
            urgency,
            priority: cells[key(impact, urgency)],
          })),
        ),
      }).unwrap();
  ...
  const handleCompute = async () => {
    ...
    setResult(null);
    try {
      const res = await computePriority({
        impact: test.impact,
  ...
  if (isLoading)
    return <div className="p-6 text-gray-500">Loading priority matrix...</div>;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <Grid3x3 className="h-6 w-6" /> Priority Matrix Editor
        </h1>
      ...
      <div className="card p-6 overflow-x-auto">
        <table className="min-w-full border-collapse">
          <thead>
            <tr>
              <th className="p-2 text-sm font-semibold text-gray-500 text-left">
                Impact ↓ / Urgency →
              </th>
              {URGENCIES.map((u) => (
                <th
                  key={u}
                  className="p-2 text-sm font-semibold capitalize text-gray-700"
                >
                  {u}
                </th>
              ))}
          ...
          <tbody>
            {IMPACTS.map((i) => (
              <tr key={i}>
                <td className="p-2 text-sm font-semibold capitalize text-gray-700">
                  {i}
                </td>
                {URGENCIES.map((u) => (
                  <td key={u} className="p-2">
                    <select
                      ...
                      onChange={(e) =>
                        setCells({ ...cells, [key(i, u)]: e.target.value })
                      }
      ...
      <div className="card p-6">
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div>
            ...
            <select
              value={test.impact}
              onChange={(e) => setTest({ ...test, impact: e.target.value })}
          ...
          <div>
            ...
            <select
              value={test.urgency}
              onChange={(e) => setTest({ ...test, urgency: e.target.value })}
              className="mt-1 input-field capitalize"
            >
              {URGENCIES.map((u) => (
        ...
        {result && (
          <div className="mt-4 rounded-lg bg-gray-50 border p-4 text-sm flex flex-wrap items-center gap-4">
            ...
            <span className="text-gray-500">
              source:{" "}
              <code>{String(result.source ?? JSON.stringify(result))}</code>
            </span>
          </div>
        )}
      </div>
