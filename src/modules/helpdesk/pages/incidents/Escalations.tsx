      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Priority, SLA &amp; functional escalation — evaluated by the engine
          </p>
        </div>
        {canManage ? (
          ...
          <span
            ...
            title="Requires escalations.manage"
          >
            Read-only (no escalations.manage)
          </span>
        )}
      </div>
      ...
      {showForm && canManage && (
        <form onSubmit={save} className="card p-6 space-y-4">
          <h2 className="font-semibold">
            {editing ? "Edit rule" : "New escalation rule"}
          </h2>
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <input
                type="number"
                min={0}
                value={form.overdueMinutes}
                onChange={(e) =>
                  setForm({ ...form, overdueMinutes: Number(e.target.value) })
                }
          ...
          <div className="grid md:grid-cols-2 gap-4 bg-gray-50 border rounded-lg p-4">
            <select
              value={form.raisePriorityTo}
              onChange={(e) =>
                setForm({ ...form, raisePriorityTo: e.target.value })
              }
              className={sel}
            >
            ...
            <select
              value={form.reassignAgent}
              onChange={(e) =>
                setForm({ ...form, reassignAgent: e.target.value })
              }
              className={sel}
            >
            ...
            <select
              value={form.reassignTeam}
              onChange={(e) =>
                setForm({ ...form, reassignTeam: e.target.value })
              }
              className={sel}
            >
              ...
              {teams.map((t: any) => (
                <option key={t._id} value={t._id}>
                  {t.name} (lead notified)
                </option>
              ))}
            </select>
            <select
              value={form.notifyAgent}
              onChange={(e) =>
                setForm({ ...form, notifyAgent: e.target.value })
              }
              className={sel}
            >
              ...
              items.map((r) => (
                <tr key={r._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-xs text-gray-500">
                    {[
                      r.action?.raisePriorityTo &&
                        `↑ ${r.action.raisePriorityTo}`,
                      r.action?.reassignAgent &&
                        `→ ${nameOf(r.action.reassignAgent)}`,
                      r.action?.reassignTeam &&
                        `→ team ${nameOf(r.action.reassignTeam)}`,
                      r.action?.notifyAgent &&
                        `🔔 ${nameOf(r.action.notifyAgent)}`,
                    ]
                      .filter(Boolean)
                      .join(" · ") || "—"}
                  </td>
