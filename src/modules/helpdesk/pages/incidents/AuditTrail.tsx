...
export default function AuditTrail() {
  ...
  const actions = useMemo(
    () =>
      Array.from(
        new Set(events.map((e) => e.action).filter(Boolean)),
      ) as string[],
    [events],
  );
  ...
  const rows = events.filter((e) => {
    ...
    if (
      ...
      !(e.resourceId || "").toLowerCase().includes(resourceFilter.toLowerCase())
    )
      return false;
    return true;
  });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Record, state, assignment &amp; field-change history
        </p>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <input
          ...
          placeholder="Filter by resource type / id…"
                      ...
                      <td colSpan={5} className="px-5 py-3">
                        <div className="grid md:grid-cols-2 gap-3 text-xs">
                          <div>
                            ...
                            <pre className="bg-white border rounded p-2 max-h-40 overflow-auto">
