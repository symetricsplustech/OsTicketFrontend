...
import { incidentApi, IncidentCI } from '@modules/helpdesk/services/incidents/incidentApi';
...
export default function IncidentCIRelated() {
  ...
  const load = async () => {
    ...
    } catch { toast.error('Failed to load CIs'); } finally { setLoading(false); }
  };
  ...
  const handleLink = async () => {
    ...
    try {
      ...
      setCiId('');
      load();
    } catch { toast.error('Failed to link CI'); }
  };
  ...
  const handleUnlink = async (ciId: string) => {
    ...
    try {
      ...
      load();
    } catch { toast.error('Failed to unlink CI'); }
  };

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Related CIs / Service Offerings</h1>
      <div className="card p-6 space-y-4">
        <div className="flex gap-2">
          <input value={ciId} onChange={e => setCiId(e.target.value)} placeholder="CI ID" className="flex-1 border rounded px-3 py-2 text-sm" />
          <select value={role} onChange={e => setRole(e.target.value)} className="border rounded px-3 py-2 text-sm">
            <option value="primary">Primary</option>
            <option value="affected">Affected</option>
            <option value="related">Related</option>
          </select>
          <button onClick={handleLink} className="px-4 py-2 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">Link CI</button>
        </div>
      </div>
      <div className="card p-6">
        {loading ? <p>Loading...</p> : cis.length === 0 ? <p className="text-gray-500">No CIs linked</p> : (
