...
export default function Incidents() {
  ...
  const { data, isLoading, isError, error, refetch } = useListIncidentsQuery(
    undefined,
    { refetchOnMountOrArgChange: true },
  );
  const incidents: Incident[] = (data?.incidents || []) as Incident[];
  ...
  const loadError = isError
    ? (error as any)?.status === 403
      ...
      : "Unable to load incidents. Please retry."
    : "";

  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canCreate)
      return toast.error("You do not have permission to create incidents.");
    setSaving(true);
    try {
  ...
  const handleResolve = async (id: string) => {
    if (!canUpdate)
      return toast.error("You do not have permission to update incidents.");
    try {
