...
export default function KnowledgeInsights() {
  ...
  const zeroResultQueries: ZeroResultQuery[] = Array.isArray(
    gapData?.zeroResultQueries,
  )
    ? gapData.zeroResultQueries
    : [];
  ...
  const runSweep = async () => {
    ...
    try {
      ...
      setSweepErr(
        err?.data?.error || err?.data?.message || "Publish sweep failed.",
      );
    } finally {
      setSweepBusy(false);
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
          ...
          <p className="text-sm text-gray-500">
            Scan drafts and publish everything ready for the knowledge base.
          </p>
          <button
            onClick={runSweep}
            disabled={sweepBusy}
          ...
          >
            ...
            {sweepBusy ? "Publishing…" : "Publish sweep"}
          </button>
          {sweepErr && <p className="text-xs text-red-600">{sweepErr}</p>}
          {sweepResult && (
            <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3">
              <p className="text-sm font-medium text-green-800">
                {sweepResult.publishedNow ?? 0} articles published now
              </p>
              {typeof sweepResult.skipped === "number" && (
              ...
              <tbody className="divide-y divide-gray-100">
                {zeroResultQueries.map((q, i) => (
                  <tr key={i} className="hover:bg-gray-50/60">
                    <td className="py-2 text-gray-800 truncate max-w-[220px]">
                      {q.query || "—"}
                    </td>
