...
export default function ArticleRecord() {
    ...
    try {
      const [artRes, verRes, comRes, fbRes, ratRes, metRes] = await Promise.all(
        [
          ...
          knowledgeApi.listVersions(id),
          knowledgeApi.listComments(id),
          knowledgeApi.listFeedback(id),
          knowledgeApi.getRatings(id),
          knowledgeApi.getMetrics(id),
        ],
      );
  ...
  const handleTransition = async (lifecycle: string) => {
    if (!id) return;
    await knowledgeApi.transitionArticle(id, { lifecycle });
    loadData();
  };
  const handleVote = async (helpful: boolean) => {
    if (!id) return;
    await knowledgeApi.createFeedback(id, {
  ...
  const handleRate = async (rating: number) => {
    ...
    await knowledgeApi.rateArticle(id, rating);
    loadData();
  };
  const handleComment = async () => {
    if (!id || !newComment.trim()) return;
    await knowledgeApi.createComment(id, { content: newComment });
    setNewComment("");
    loadData();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold">{article.question}</h1>
          <p className="text-muted-foreground">
            {article.number} | v{article.version}
          </p>
        </div>
        <Badge className={lifecycleColors[article.lifecycle]}>
          {article.lifecycle}
        </Badge>
        {NEXT_STEPS[article.lifecycle]?.map((step) => (
          <Button
            key={step}
            size="sm"
            variant="outline"
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span className="flex items-center gap-1">
          <Eye className="h-4 w-4" /> {metrics?.views || 0}
        </span>
        <span className="flex items-center gap-1">
          <ThumbsUp className="h-4 w-4" /> {article.helpful}
        </span>
        <span className="flex items-center gap-1">
          <ThumbsDown className="h-4 w-4" /> {article.notHelpful}
        </span>
        <span className="flex items-center gap-1">
          <Star className="h-4 w-4" /> {ratings?.averageRating || 0} (
        ...
        <span className="flex items-center gap-1">
          <History className="h-4 w-4" /> {versions.length}
        </span>
      </div>
      <div className="flex gap-2">
        ...
        <div className="flex items-center gap-1 ml-4">
          {[1, 2, 3, 4, 5].map((star) => (
            <Star
              key={star}
              className={`h-5 w-5 cursor-pointer ${star <= userRating ? "text-yellow-500 fill-yellow-500" : "text-gray-300"}`}
              ...
              <CardContent className="flex items-center justify-between py-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {v.changeType} at {new Date(v.createdAt).toLocaleString()}
                  </p>
                </div>
                <Badge
                  className={lifecycleColors[v.lifecycle] || "bg-gray-100"}
                >
                  {v.lifecycle}
                </Badge>
              </CardContent>
              ...
              <CardContent className="py-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium text-sm">
                    {(c.userId as unknown as { name: string })?.name}
                  </span>
            ...
            <Card key={f._id}>
              <CardContent className="py-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-medium text-sm">
                    {(f.userId as unknown as { name: string })?.name}
                  </span>
                  <Badge
                    className={
                      f.type === "helpful"
