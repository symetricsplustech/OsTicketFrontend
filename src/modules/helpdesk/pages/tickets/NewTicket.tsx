...
export default function NewTicket() {
  ...
  const [suggestions, setSuggestions] = useState<
    Array<{ _id: string; question: string; helpful?: number }>
  >([]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await api.get("/kb/suggest", {
          params: { q: q.slice(0, 200) },
        });
      ...
      >
        {suggestions.length > 0 && !solved && (
          <div className="rounded-lg border border-brand-200 bg-brand-50/50 p-4">
            <p className="text-sm font-semibold text-gray-800">
              Suggested solutions — did one of these fix it?
            </p>
            <ul className="mt-2 space-y-1">
              {suggestions.map((s) => (
                <li key={s._id} className="text-sm">
                  <Link to={`/kb`} className="text-brand-700 hover:underline">
                    {s.question}
                  </Link>
                  {!!s.helpful && (
                    <span className="text-xs text-gray-400">
                      {" "}
                      · {s.helpful} found helpful
                    </span>
                  )}
                </li>
              ))}
        ...
        {solved && (
          <div className="rounded-lg border border-green-200 bg-green-50 p-4 text-sm text-green-800">
            Glad the knowledge base helped! You can still submit below if the
            issue persists.
          </div>
        )}
        <div>
          ...
          {triage && (
            <div className="mt-2 space-y-1.5 bg-brand-50/50 border border-brand-100 rounded-lg p-3">
              <p className="text-[11px] text-gray-500">
                ...
                {!triage.learned && "(not enough history yet)"}
              </p>
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            …or let the matrix decide from impact × urgency:
          </p>
