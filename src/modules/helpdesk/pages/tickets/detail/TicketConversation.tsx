...
export function TicketConversation({ entries, selecting, selectedIds, onToggle }: {
  ...
  return (
    <div className="space-y-4">
      {entries.map((entry) => (
        <Card key={entry._id} className={`p-5 ${entry.type === 'note' ? 'border-l-4 border-yellow-400 bg-yellow-50/30' : ''}`}>
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {entry.type === 'note' && <span className="rounded bg-yellow-100 px-2 py-0.5 text-xs text-yellow-700">Internal Note</span>}
            </div>
          ...
          {!!entry.attachments?.length && (
            <div className="mt-3 flex flex-wrap gap-2">
              {entry.attachments.map((attachment) => (
                <a key={attachment} href={attachment} target="_blank" rel="noreferrer" className="flex items-center gap-1 rounded bg-gray-100 px-2 py-1 text-xs text-gray-600 hover:bg-gray-200">
...
export function TicketSummary({ summary, onDismiss }: { summary: any; onDismiss: () => void }) {
  ...
  return (
    <Card className="border-brand-200 bg-brand-50/40 p-5">
      <div className="mb-2 flex items-center gap-2">
        ...
        <Button variant="ghost" size="sm" className="ml-auto" onClick={onDismiss}>Dismiss</Button>
      </div>
      <p className="whitespace-pre-wrap text-sm text-gray-700">{summary.summary || 'Nothing substantial yet.'}</p>
