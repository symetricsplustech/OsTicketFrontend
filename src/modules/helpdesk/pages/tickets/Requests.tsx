...
export default function Requests() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setChains(
        Array.isArray(cRes.data) ? cRes.data : cRes.data?.records || [],
      );
    } catch (error: any) {
      setBundles([]);
      setChains([]);
  ...
  const checkout = async () => {
    ...
    try {
      ...
      const kids =
        ...
        res.data?.requestedItems?.length ??
        cart.length;
      toast.success(`Request placed — ${kids} requested item(s) created`);
      setCart([]);
    } catch (e: any) {
      toast.error(
        e?.response?.data?.error || "Checkout failed (quota / eligibility?)",
      );
    }
  };
  ...
  const decide = async (id: string, verdict: "approved" | "rejected") => {
    if (!canDecideApprovals)
      return toast.error("You do not have permission to decide approvals.");
    try {
      await api.post(`/gaps2/approval-chains/${id}/decide`, {
        decision: verdict,
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Catalog checkout, requested items &amp; approval chains
          </p>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="card p-5">
          ...
          <div className="flex gap-2 mb-2">
            ...
            />
            <button onClick={addToCart} className="btn-secondary text-xs">
              Add
            </button>
          </div>
          <input
            value={fulfilFor}
            onChange={(e) => setFulfilFor(e.target.value)}
            placeholder="Fulfil for (user id, optional)"
            className="input-field text-sm w-full mb-3"
          />
          {cart.length === 0 ? (
            <p className="text-xs text-gray-400">
              Cart is empty. Quotas are enforced per item at checkout.
            </p>
          ) : (
              ...
              {cart.map((c, i) => (
                <li key={i} className="py-1.5 flex justify-between">
                  <span className="font-mono text-xs">
                    {c.catalogItemId} × {c.quantity}
                  </span>
                  <button
                    onClick={() =>
                      setCart((cc) => cc.filter((_, j) => j !== i))
                    }
      ...
      {canManageApprovals && (
        <div className="card overflow-hidden">
          <div className="px-5 py-3 border-b">
            <h2 className="font-semibold text-sm">Approval chains</h2>
          </div>
          {loading ? (
