    ...
    const load = async () => {
      try {
        ...
        setStats({
          ...
          open: items.filter((p: Problem) =>
            ["new", "assess"].includes(p.status),
          ).length,
          analyzing: items.filter(
            (p: Problem) => p.status === "root_cause_analysis",
          ).length,
          fixed: items.filter((p: Problem) =>
            ["resolved", "closed"].includes(p.status),
          ).length,
          ...
          {recentProblems.map((prob) => (
            ...
            >
              ...
              <div className="flex gap-2">
                <span
                  className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[prob.status] || "bg-gray-100"}`}
                >
