...
export default function KnownErrors() {
  ...
  const [filter, setFilter] = useState<"all" | "with-workaround" | "no-rca">(
    "all",
  );
    ...
    try {
      ...
      toast.error(
        error?.response?.status === 403
          ? "You do not have permission to publish knowledge."
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Root cause, workarounds, KB publishing &amp; permanent fixes
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Link to="/kb" className="btn-secondary text-sm">
            Knowledge Base
          </Link>
        </div>
      </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                      ...
                      <p className="text-gray-700">
                        {p.rootCause || (
                          <span className="text-gray-400">
                            RCA pending — five-whys analysis needed
                          </span>
                        )}
                      </p>
                    </div>
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    {p.assignedTo?.name ? `Owner: ${p.assignedTo.name} · ` : ""}
                    Opened {formatDate(p.createdAt)}
                  </p>
                </div>
                <div className="flex flex-col gap-2 shrink-0">
                  {canPublishKnowledge && (
                    <button
                      ...
                      title={
                        !p.workaround
                          ? "Add a workaround before publishing"
