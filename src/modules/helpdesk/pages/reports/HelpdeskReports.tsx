...
export default function HelpdeskReports() {
  ...
  const cards = [
    ...
    {
      label: "Open",
      value: overview?.open ?? realtime?.openTickets ?? "—",
      tone: "text-blue-600",
    },
    {
    ...
    },
    {
      ...
      value: realtime?.openIncidents ?? "—",
      tone: "text-red-600",
    },
    {
      label: "Pending changes",
      value: realtime?.pendingChanges ?? "—",
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Backlog, resolution, response times &amp; knowledge gaps
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {cards.map((c) => (
          <div key={c.label} className="bg-white rounded-xl border p-4">
            <p className={`text-2xl font-bold ${c.tone}`}>
              {loading ? "…" : c.value}
            </p>
          ...
          {mtt ? (
            <dl className="text-sm space-y-2">
              <div className="flex justify-between">
                <dt className="text-gray-500">Alerts tracked</dt>
                <dd className="font-medium">{mtt.alertsTracked ?? "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">Mean time to acknowledge</dt>
                <dd className="font-medium">
                  {mtt.mttAckMinutes != null
                    ? `${Math.round(mtt.mttAckMinutes)} min`
                    : "—"}
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">Mean time to resolve</dt>
                <dd className="font-medium">
                  {mtt.mtrMinutes != null
                    ? `${Math.round(mtt.mtrMinutes)} min`
                    : "—"}
                </dd>
              </div>
              {mtt.note && (
                <p className="text-xs text-gray-400 pt-1">{mtt.note}</p>
              )}
            </dl>
          ) : (
            <p className="text-xs text-gray-400">
              No MTT metrics available yet.
            </p>
          )}
          <Link
