...
export default function MyWork() {
  ...
  const buckets = useMemo(() => {
    const open = tickets.filter(
      (t) => !["closed", "resolved", "archived"].includes(t.status),
    );
                ...
                return (
                  <tr key={t._id} className="hover:bg-gray-50">
                    <td className="px-5 py-3">
                      ...
                      >
                        #{t.number} —{" "}
                        {t.title || (t as any).subject || "Untitled"}
                      </Link>
                    </td>
                    <td className="px-5 py-3">
        ...
        <div className="bg-white rounded-xl border p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {saved.map((s: any) => (
              <span
                key={s._id || s.name}
                className="inline-flex items-center gap-2 px-3 py-1.5 bg-gray-50 border rounded-full text-xs"
              >
                <button
