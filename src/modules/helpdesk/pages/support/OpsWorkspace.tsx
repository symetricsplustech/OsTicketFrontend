...
function OpsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await api.get("/enterprise/itom/alerts", {
  ...
  const ackAlert = async (id: string) => {
    try {
      await api.put(`/enterprise/itom/alerts/${id}`, {
        status: "acknowledged",
      });
    ...
    <div className="min-h-screen bg-gray-50 p-6 space-y-6">
        ...
        <div className="card overflow-hidden lg:col-span-2">
          <div className="px-5 py-3 border-b flex items-center justify-between">
            <h2 className="font-semibold text-sm flex items-center gap-2">
              <AlertTriangle size={14} className="text-orange-500" /> Firing
              Alerts
            </h2>
            <span className="text-xs text-gray-400">
              {fireAlertCount} active
            </span>
          </div>
                ...
                {alerts.map((a) => (
                  <tr key={a._id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${SEVERITY_COLORS[a.severity] || "bg-gray-100 text-gray-700"}`}
                      >
                        {a.severity}
                      </span>
                    </td>
              ...
              {incidents.map((inc) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${PRIORITY_COLORS[inc.priority || inc.severity || "medium"] || "bg-gray-100 text-gray-700"}`}
                    >
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-3 border-b flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">
            {slaAtRiskCount} at risk · {watching.length} watching
          </span>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {[...atRisk, ...watching].map((p) => (
                <tr key={p.ticketId || p.number} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-sm truncate max-w-xs">
                    {p.subject}
                  </td>
                  <td className="px-5 py-3">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${PRIORITY_COLORS[p.priority] || "bg-gray-100 text-gray-700"}`}
                    >
                      {p.priority}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${CATEGORY_COLORS[p.category] || "bg-gray-100 text-gray-700"}`}
                    >
                      {p.category}
                    </span>
                  </td>
