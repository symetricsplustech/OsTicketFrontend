...
export default function SupportEmail() {
  ...
  const confirmChange = async (e: React.FormEvent) => {
    ...
    setConfirming(true);
    try {
      const res = await api.post("/users/me/email/confirm", {
        token: token.trim(),
      });
  ...
  const saveInbox = async (e: React.FormEvent) => {
    ...
    setSavingInbox(true);
    try {
      const res = await api.put("/users/company/support-inbox", {
        supportEmail: inbox.trim(),
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Mail from this address to create tickets without logging in. They
      ...
      <div className="card p-5 space-y-2">
        <div className="text-sm">
          <span className="text-gray-500">Your sender address: </span>
          <span className="font-semibold">{senderAddress || "—"}</span>
        </div>
        <div className="text-sm">
          <span className="text-gray-500">
            Mail tickets to (hired organisation):{" "}
          </span>
          {status?.supportInbox ? (
            ...
            <span className="text-gray-400">
              not configured — ask your company owner to set it below
            </span>
          )}
          {status?.supportCompany && (
            <span className="text-gray-500">
              {" "}
              ({status.supportCompany.name})
            </span>
          )}
        </div>
        {ticketCount !== null && (
          <div className="text-sm text-gray-500">
            {ticketCount} ticket(s) linked to this account.{" "}
            <Link to="/ticket-board" className="text-brand-600 hover:underline">
              View progress →
            </Link>
          </div>
        )}
        <p className="text-xs text-gray-400">
          Tip: keep the ticket <span className="font-mono">[#NUMBER]</span> in
          the subject when replying by email so it threads to the same ticket.
        </p>
      </div>
      ...
      {status?.canConfigure && (
        <div className="card p-5 border-brand-200">
          ...
          <p className="text-xs text-gray-500 mb-3">
            You own this company — set the ONE address customers mail to.
            Inbound mail to it auto-creates tickets here.
          </p>
          <form onSubmit={saveInbox} className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">
                ...
                <span className="font-normal text-gray-400">
                  (optional fallback: anything@mail-domain routes here)
                </span>
              </label>
              <input
            ...
            <button disabled={savingInbox} className="btn-primary text-sm">
              {savingInbox ? "Saving…" : "Save inbox"}
            </button>
          </form>
        </div>
      )}
      ...
      {status?.pendingEmail && (
        <div className="card p-5 border-amber-200 bg-amber-50">
          <p className="text-sm font-medium text-amber-800">
            Pending verification: {status.pendingEmail}
          </p>
          <p className="text-xs text-amber-600">
            Confirm within 30 minutes using the token mailed to the new address.
          </p>
        </div>
      )}
      ...
      <div className="card p-5">
        ...
        <p className="text-xs text-gray-400 mt-2">
          History stays linked — the account id never changes, only the address.
        </p>
      </div>
      ...
      <div className="card p-5">
        ...
        <form onSubmit={confirmChange} className="flex gap-2">
          ...
          />
          <button disabled={confirming} className="btn-secondary">
