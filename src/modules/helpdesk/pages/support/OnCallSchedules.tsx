...
export default function OnCallSchedules() {
  ...
  const loadSchedules = async () => {
    try {
      const { data } = await api.get("/platform/oncall");
      setSchedules(data);
    } catch {}
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post("/platform/oncall", form);
      setShowForm(false);
      setForm({
        name: "",
        description: "",
  ...
  const handleEscalate = async (id: string) => {
    try {
      ...
      setModal({
        ...
        message: `Current on-call: ${data.currentOnCall?.name || "None"}`,
      });
    } catch {}
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Users className="h-6 w-6" /> On-Call Schedules
        </h1>
        <button
        ...
        {schedules.map((s) => (
          <div key={s._id} className="bg-white p-4 rounded-lg border">
            <div className="flex items-start justify-between">
              ...
              <span
                className={`px-2 py-1 text-xs rounded-full ${s.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
              >
                {s.status}
              </span>
            </div>
                ...
                {s.escalation
                  ...
                  .map((e, i) => (
                    <div key={i} className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-400 text-xs">
                        after {e.delayMinutes}min via {e.notifyMethod}
                      </span>
                    </div>
                  ))}
              </div>
