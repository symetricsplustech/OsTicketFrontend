...
export default function ServiceCatalog() {
  ...
  const openRequest = (item: CatalogItem) => {
    setShowRequest(item);
    api
      .get("/ops/custom-forms/for-item/" + item._id)
      .then((r) => {
        setDynamicForm(r.data);
        setDynValues({});
      })
      .catch(() => setDynamicForm(null));
  };
  ...
  const handleRequest = async (e: React.FormEvent) => {
    ...
    if (
      dynamicForm?.fields?.some(
        (f: any) =>
          f.required &&
          conditionVisible(f, dynValues) &&
          (dynValues[f.name] === undefined ||
            dynValues[f.name] === "" ||
            dynValues[f.name] === false),
      )
    ) {
    ...
    try {
      await api.post("/tickets", {
        ...
        customFields: {
          catalogItemId: showRequest._id,
          quantity: Number(requestForm.quantity),
          ...(Object.keys(dynValues).length
            ? { dynamicFormData: dynValues }
            : {}),
        },
      });
  ...
  return (
    <div className="space-y-6">
          ...
          <form onSubmit={handleRequest} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder={`Request: ${showRequest.name}`}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">
      ...
      <div className="flex gap-3">
        <button
          onClick={() => setCategory("")}
          className={`px-3 py-1.5 rounded-lg text-sm font-medium ${!category ? "bg-brand-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
        >
          All
        </button>
        {categories.map((c) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium ${category === c ? "bg-brand-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
          >
            {c}
          </button>
        ))}
