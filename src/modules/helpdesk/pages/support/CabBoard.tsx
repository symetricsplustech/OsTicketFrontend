...
export default function CabBoard() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setPending(
        all.filter(
          (c) => c.status === "for_approval" || c.status === "pending_approval",
        ),
      );
      setBlackouts(
        Array.isArray(bRes.data) ? bRes.data : bRes.data?.records || [],
      );
    } catch {
      setPending([]);
    } finally {
  ...
  const emergencyApprove = async (id: string) => {
    const reason =
      decision[id] ||
      window.prompt("Emergency justification (required):") ||
      "";
    if (!reason.trim())
      return toast.error("Justification required for emergency approval");
    try {
      await api.put(`/enterprise/changes/${id}`, {
        status: "approved",
        type: "emergency",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Change approvals, emergency path &amp; blackout windows
          </p>
        </div>
        <div className="flex gap-2">
          <Link to="/changes" className="btn-secondary text-sm">
            All Changes
          </Link>
          <Link to="/change-calendar" className="btn-secondary text-sm">
            Change Calendar
          </Link>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-3 border-b">
          <h2 className="font-semibold text-sm">
            Awaiting CAB decision ({pending.length})
          </h2>
        </div>
            ...
            {loading ? (
              ...
              pending.map((c) => (
                <tr key={c._id} className="hover:bg-gray-50 align-top">
                  ...
                  <td className="px-5 py-3">
                    <StatusBadge status={c.type || "normal"} />
                  </td>
                  <td className="px-5 py-3">
                    <StatusBadge status={c.risk || c.riskLevel || "medium"} />
                  </td>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`text-sm font-bold ${(c.riskScore ?? 0) >= 60 ? "text-red-600" : (c.riskScore ?? 0) >= 30 ? "text-orange-600" : "text-green-600"}`}
                      >
                        {c.riskScore ?? "—"}
                      </span>
                      <button
                    ...
                    {conflicts[c._id] && (
                      <div className="text-[11px] text-gray-500 mt-1">
                        {(conflicts[c._id].overlapping || []).map((o: any) => (
                          <p key={o.id}>⚠️ {o.number}</p>
                        ))}
                        {(conflicts[c._id].blackouts || []).map((b: any) => (
                          <p key={b.id}>⛔ {b.name}</p>
                        ))}
                        {!conflicts[c._id].overlapping?.length &&
                          !conflicts[c._id].blackouts?.length && (
                    ...
                    {canDecide ? (
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex gap-1">
                          ...
                          />
                          <button
                            onClick={() => emergencyApprove(c._id)}
                      ...
                      <span
                        ...
                        title="Requires approvals.decide"
                      >
                        Voting restricted
                      </span>
                    )}
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            onClick={() => setShowBlackout(!showBlackout)}
        ...
        {showBlackout && (
          <form
            ...
            className="grid grid-cols-2 gap-3 mb-4 bg-gray-50 border rounded-lg p-4"
          >
            <input
              required
              value={bForm.name}
            ...
            <input
              value={bForm.reason}
              onChange={(e) => setBForm({ ...bForm, reason: e.target.value })}
              placeholder="Reason"
              className="input-field text-sm"
            />
            <input
