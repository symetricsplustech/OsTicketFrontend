...
export default function ShiftHandover() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErr(
        e2?.data?.error || e2?.data?.message || "Failed to save handover note.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <form
        onSubmit={submit}
        className="bg-white rounded-xl border border-gray-200 p-5 space-y-4 max-w-2xl"
      >
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Pending Tickets (comma separated)
          </label>
          <input
            placeholder="TKT-101, TKT-205, TKT-330"
            value={form.pendingTicketsText}
            onChange={(e) =>
              setForm({ ...form, pendingTicketsText: e.target.value })
            }
            className={inputCls}
          />
      ...
      {isLoading ? (
        <p className="bg-white rounded-xl border border-gray-200 px-6 py-12 text-center text-sm text-gray-400 animate-pulse">
          Loading handover notes…
        </p>
      ) : notes.length === 0 ? (
        <p className="bg-white rounded-xl border border-gray-200 px-6 py-12 text-center text-sm text-gray-500">
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-gray-900">
                  {n.shiftDate
                    ? new Date(n.shiftDate).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                      })
              ...
              {n.risks && (
                <p className="text-sm text-red-600 whitespace-pre-wrap">
                  <span className="font-medium">Risks:</span> {n.risks}
                </p>
              )}
              {n.notes && (
