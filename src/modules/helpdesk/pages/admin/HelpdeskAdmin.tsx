...
export default function HelpdeskAdmin() {
  ...
  useEffect(() => {
    load(tab);
    setDraft({ topic: "", title: "", message: "", name: "" });
  }, [tab]);
  ...
  const createAnnouncement = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post("/agent/announcements", {
        title: draft.title,
        body: draft.message,
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Categories, templates, codes &amp; retention
          </p>
        </div>
        <Link to="/priority-matrix" className="btn-secondary text-sm">
          Priority Matrix
        </Link>
      </div>
          ...
          {tab === "topics" && (
            <div className="card p-5 space-y-4">
              ...
              <ul className="divide-y text-sm">
                {topics.map((t: any) => (
                  <li key={t._id || t.topic} className="py-2">
                    {t.topic || t.name}
                  </li>
                ))}
                {topics.length === 0 && (
                  <li className="py-4 text-gray-400 text-sm">No topics.</li>
                )}
              </ul>
            </div>
          )}
          ...
          {tab === "canned" && (
            <div className="card p-5 space-y-4">
              ...
              <ul className="divide-y text-sm">
                ...
                {canned.length === 0 && (
                  <li className="py-4 text-gray-400 text-sm">
                    No canned responses.
                  </li>
                )}
              </ul>
            ...
            <div className="card p-5">
              {codes ? (
                <div className="grid md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(codes.resolutionCodes || []).map((c: string) => (
                        <span
                          key={c}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(codes.closureCodes || []).map((c: string) => (
                        <span
                          key={c}
                ...
                <p className="text-sm text-gray-400">
                  Closure codes unavailable.
                </p>
              )}
              <p className="text-xs text-gray-400 mt-4">
                Set per ticket via{" "}
                <span className="font-mono">
                  PUT /gaps2/tickets/:number/closure
                </span>{" "}
                or the ticket detail screen.
              </p>
            </div>
          )}
                ...
                <tbody className="divide-y divide-gray-100">
                  {trash.length === 0 ? (
                    <tr>
                      ...
                      >
                        Trash is empty — soft-deleted tickets appear here, never
                        hard-deleted.
                      </td>
                    </tr>
                  ) : (
                    trash.map((t: any, i: number) => (
                      <tr key={i} className="hover:bg-gray-50">
                        ...
                        <td className="px-5 py-2.5">
                          <button
                            onClick={() => restore(t.ticketNumber)}
