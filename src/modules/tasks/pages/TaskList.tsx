    ...
    <div className="space-y-6">
            ...
            <div className="flex gap-3 mt-2 text-sm text-gray-500">
              {stats.byState.map((s) => (
                <span
                  ...
                  onClick={() =>
                    setStateFilter(s._id === stateFilter ? "" : s._id)
                  }
                >
                  {s._id}: {s.count}
                </span>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {["task", "incident", "problem", "change", "request", "subtask"].map(
            (t) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {tasks.map((task) => (
                <tr key={task._id} className="hover:bg-gray-50 cursor-pointer">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${PRIORITY_COLORS[task.priority] || "bg-gray-100"}`}
                    >
                      {task.priority}
                    </span>
                  </td>
