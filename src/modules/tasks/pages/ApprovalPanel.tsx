          ...
          {pendingApprovals.length > 0 && (
            <div>
              ...
              {pendingApprovals.map((a) => (
                <div
                  key={a._id}
                  className="p-3 rounded-lg border border-yellow-200 bg-yellow-50 mb-2"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      {STATE_ICONS[a.state]}
                      <span className="text-sm font-medium">
                        {a.type === "group" || a.type === "parallel"
                          ? "Group"
                          : "Individual"}{" "}
                        Approval
                      </span>
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATE_COLORS[a.state]}`}
                      >
                        {a.state}
                      </span>
                    </div>
                  ...
                  {a.approvalGroup && (
                    <div className="mt-1 text-xs text-gray-600">
                      Group: {a.approvalGroup.name}
                    </div>
                  )}
                  {a.type === "parallel" && (
                    <div className="mt-1 text-xs text-gray-500">
                      <Users className="inline h-3 w-3" />{" "}
                      {a.approvalsReceived?.length || 0}/{a.requiredApprovals}{" "}
                      approvals received
                    </div>
                  )}
                  {!isTerminal && (
          ...
          {decidedApprovals.length > 0 && (
            <div>
              ...
              {decidedApprovals.map((a) => (
                <div
                  key={a._id}
                  className="p-2 rounded-lg border border-gray-100 mb-2"
                >
                  <div className="flex items-center gap-2">
                    {STATE_ICONS[a.state]}
