...
export const taskApi = {
  ...
  create: (data: Partial<Task>) => api.post<Task>(BASE, data),
  ...
  restore: (id: string) => api.post<Task>(`${BASE}/${id}/restore`),
  ...
  getStats: () => api.get<TaskStats>(`${BASE}/stats`),

  getWatchers: (id: string) => api.get<TaskWatcher[]>(`${BASE}/${id}/watchers`),
  ...
  getRelationships: (id: string) =>
    api.get<{ outgoing: TaskRelationship[]; incoming: TaskRelationship[] }>(
      `${BASE}/${id}/relationships`,
    ),
  ...
  addRelationship: (
    ...
    api.post<TaskRelationship>(`${BASE}/${id}/relationships`, {
      targetTaskId,
      relationshipType,
    }),
  ...
  listAttachments: (taskId: string) => api.get(`/core/attachments/${taskId}`),

  downloadAttachment: (id: string) =>
    api.get(`/core/attachments/${id}/download`, { responseType: "blob" }),

  removeAttachment: (id: string) => api.delete(`/core/attachments/${id}`),
};

// Approvals API
export const approvalApi = {
  listForTask: (taskId: string) => api.get(`/core/approvals/task/${taskId}`),

  create: (data: Record<string, unknown>) => api.post("/core/approvals", data),

  decide: (id: string, decision: string, note = "") =>
    api.post(`/core/approvals/${id}/decide`, { decision, note }),

  cancel: (id: string) => api.post(`/core/approvals/${id}/cancel`),

  delegate: (id: string, delegatedTo: string, note = "") =>
    api.post(`/core/approvals/${id}/delegate`, { delegatedTo, note }),

  pending: () => api.get("/core/approvals/pending"),

  stats: () => api.get("/core/approvals/stats"),
};
...
export const slaApi = {
  listForTask: (taskId: string) => api.get(`/core/tasks/${taskId}`),
};
