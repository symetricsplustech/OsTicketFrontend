...
export default function SlaPlans() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.post("/admin/sla-plans", {
        ...form,
        gracePeriod: Number(form.gracePeriod),
        targets: {
          assignment: Number(form.assignmentMinutes) / 60,
          first_response: Number(form.responseMinutes) / 60,
          update: Number(form.updateMinutes) / 60,
          resolution: Number(form.resolutionMinutes) / 60,
        },
      });
      ...
      toast.error("Failed to create SLA plan");
    } finally {
      setSaving(false);
    }
  };
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-2 gap-4">
            ...
            {(
              [
                ...
                ["First response", "responseMinutes"],
                ["Update frequency", "updateMinutes"],
            ...
            <div className="flex items-end gap-2 col-span-2">
              <button type="submit" disabled={saving} className="btn-primary">
                {saving ? "Creating..." : "Create SLA Plan"}
              </button>
              <button
                type="button"
              ...
              plans.map((p) => (
                <tr key={p._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 text-xs rounded-full ${p.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"}`}
                    >
                      {p.status}
                    </span>
                  </td>
