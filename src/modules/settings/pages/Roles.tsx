...
export default function Roles() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/admin/roles", {
        ...form,
        approvalLimit: form.approvalLimit ? Number(form.approvalLimit) : null,
  ...
  const [permissionEditing, setPermissionEditing] = useState<string | null>(
    null,
  );
  ...
  const saveScope = async (roleId: string) => {
    try {
      ...
      toast.error("Failed to save module scope");
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Tenant roles only. Platform roles are managed separately and can never
          be assigned here.
        </p>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={form.recordScopes[0]}
                  onChange={(e) =>
                    setForm({ ...form, recordScopes: [e.target.value] })
                  }
                  className="mt-1 input-field"
                >
                  {[
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, approvalLimit: e.target.value })
                  }
                  className="mt-1 input-field"
                  placeholder="No approval authority"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">
                    {permissionEditing === r._id ? (
                      <div className="min-w-[48rem] space-y-2">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => savePermissions(r._id)}
                            className="btn-primary text-xs px-2 py-1"
                          >
                            Save
                          </button>
                          <button
                            onClick={() => setPermissionEditing(null)}
                      ...
                      >
                        {r.permissions?.length || 0} permissions · Edit
                      </button>
                    )}
                  </td>
                  <td className="px-6 py-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex flex-wrap gap-1 max-w-md">
                          {ALL_MODULES.map((m) => (
                            <label
                              key={m}
                              className={`text-xs px-2 py-1 rounded cursor-pointer border ${scopeSelection.includes(m) ? "bg-brand-50 border-brand-300 text-brand-700" : "bg-gray-50 border-gray-200 text-gray-500"}`}
                            >
                              <input
                                type="checkbox"
                                className="hidden"
                                checked={scopeSelection.includes(m)}
                                onChange={() =>
                        ...
                        <div className="flex gap-1">
                          <button
                            onClick={() => saveScope(r._id)}
                            className="btn-primary text-xs px-2 py-1"
                          >
                            Save
                          </button>
                          <button
                            onClick={() => setScopeEditing(null)}
                            className="btn-secondary text-xs px-2 py-1"
                          >
                            Cancel
                          </button>
