...
export default function AdvancedViews() {
  const [getFunnelReport, { isLoading: loadingFunnel }] =
    useGetFunnelReportMutation();
  const [periodCompare, { isLoading: loadingCompare }] =
    usePeriodCompareMutation();
  ...
  const runFunnel = async () => {
    ...
    setFunnel(null);
    try {
      setFunnel(await getFunnelReport({}).unwrap());
    } catch (e: any) {
      setErr(e?.data?.message || "Failed to load funnel");
    }
  };
  const runCompare = async () => {
    ...
    setCompare(null);
    try {
      setCompare(await periodCompare().unwrap());
    } catch (e: any) {
      setErr(e?.data?.message || "Failed to compare periods");
    }
  };
  const runScorecard = async () => {
    ...
    setRagResult(null);
    try {
      setRagResult(await scorecard({ metric, value: Number(value) }).unwrap());
    } catch (e: any) {
      setErr(e?.data?.message || "Failed to score metric");
    }
  };
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <LineChart className="h-6 w-6" /> Advanced Views
        </h1>
        {err && (
          <div className="mt-2 rounded-lg bg-red-50 border border-red-200 p-3 text-sm text-red-700 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div className="space-y-3">
              {Array.isArray(stages) &&
                stages.map((s: any, i: number) => {
                  const max = Math.max(
                    ...stages.map((x: any) => x.count ?? x.value ?? 0),
                    1,
                  );
                  const w = Math.max(
                    ((s.count ?? s.value ?? 0) / max) * 100,
                    4,
                  );
                  return (
                    <div key={i}>
        ...
        <div className="card p-6 space-y-4">
                  ...
                  <tbody>
                    {rows.map((r: any, i: number) => (
                      <tr key={i} className="border-b last:border-0">
                        ...
                        <td
                          className={`py-1.5 font-semibold ${(r.deltaPct ?? r.delta ?? 0) >= 0 ? "text-green-600" : "text-red-600"}`}
                        >
                          {r.deltaPct ?? r.delta}%
                        </td>
                      </tr>
          ...
          {compare?.freshness != null && (
            <p className="text-xs text-gray-400">
              Data freshness: {new Date(compare.freshness).toLocaleString()}
            </p>
          )}
        </div>
        ...
        <div className="card p-6 space-y-4">
          ...
          <input
            type="text"
            required
            placeholder="Metric name (e.g. csat)"
            value={metric}
          ...
          <button
            ...
            disabled={!metric || value === "" || loadingScore}
          ...
          {ragResult && (
            <div className="rounded-lg bg-gray-50 border p-4 space-y-2">
              <p className="text-sm font-medium text-gray-700">
                {(ragResult as any).metric || metric}:{" "}
                <span className="font-bold">
                  {(ragResult as any).value ?? value}
                </span>
              </p>
              <span
                className={`inline-block rounded-full px-3 py-1 text-sm font-bold uppercase ${ragClass}`}
              >
                {(ragResult as any).rag || "red"}
              </span>
            </div>
          )}
        </div>
