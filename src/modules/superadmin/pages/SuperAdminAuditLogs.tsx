...
export default function SuperAdminAuditLogs() {
  ...
  const { data, isLoading } = useGetSaAuditLogsQuery({
    page,
    limit: 25,
  ...
  const handleExport = () => {
    ...
    window.open(
      `/api/v1/superadmin/audit-logs/export?${params.toString()}`,
      "_blank",
    );
  };

  const actionColor = (a: string) => {
    if (a.includes("create") || a.includes("activated"))
      return "bg-green-100 text-green-700";
    if (
      a.includes("delete") ||
      a.includes("terminated") ||
      a.includes("suspended")
    )
      return "bg-red-100 text-red-700";
    if (a.includes("update") || a.includes("modified"))
      return "bg-blue-100 text-blue-700";
    if (a.includes("login") || a.includes("impersonat"))
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Tenant lifecycle, operator actions, security events, and compliance
            review
          </p>
        </div>
        <button
                ...
                <tr key={log._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${actionColor(log.action)}`}
                      >
                        {log.action}
                      </span>
                      {log.source === "platform" && (
