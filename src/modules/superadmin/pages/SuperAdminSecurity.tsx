    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Review privileged roles, revoke sessions, and manage tenant security
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border overflow-hidden">
        <div className="px-5 py-4 border-b">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Agents with admin or elevated privileges across tenants
          </p>
        </div>
        {isLoading ? (
          <div className="p-10 text-center text-sm text-gray-400">
            Loading privileged roles...
          </div>
        ) : admins.length === 0 ? (
          <div className="p-10 text-center text-sm text-gray-400">
            ...
            <tbody className="divide-y">
              {admins.map((admin: any) => (
                <tr key={admin._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${admin.isAdmin ? "bg-red-100 text-red-700" : "bg-blue-100 text-blue-700"}`}
                    >
                      {admin.role?.name || (admin.isAdmin ? "Admin" : "Agent")}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-gray-500">
                    {admin.company || "Platform"}
                  </td>
                  <td className="px-5 py-3 text-gray-500">
                    {admin.lastLogin
                  ...
                  <td className="px-5 py-3 text-right">
                    <button
                      onClick={() => {
                        if (confirm(`Force logout ${admin.name}?`))
                          forceLogout({
                            userId: admin._id,
                            reason: "Admin action",
                          });
