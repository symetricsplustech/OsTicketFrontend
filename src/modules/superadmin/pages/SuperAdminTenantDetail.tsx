...
export default function SuperAdminTenantDetail() {
  ...
  if (!d)
    return (
      <div className="p-10 text-center text-gray-400">Tenant not found</div>
    );
  ...
  const handleSaveModules = async () => {
    try {
      ...
      setModal({
        open: true,
        title: "Error",
        message: err?.data?.message || "Failed to update modules",
      });
    }
  };
  ...
  const handleImpersonate = async () => {
    const reason = window
      .prompt("Enter the support reason for impersonating this tenant:")
      ?.trim();
    if (!reason) return;
    try {
      ...
      if (currentToken)
        localStorage.setItem("platformOriginalToken", currentToken);
      if (currentUser)
        localStorage.setItem("platformOriginalUser", currentUser);
      localStorage.setItem("token", result.token);
      localStorage.setItem(
        "privilegedSession",
        JSON.stringify({ ...result.session, reason, tenantName: d.name }),
      );
      localStorage.setItem(
        "user",
        JSON.stringify({ user: { ...result.user, role: "admin" }, tenant: d }),
      );
      ...
      setModal({
        ...
        message: err?.data?.message || "Unable to start the privileged session",
      });
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => {
                if (confirm("Delete this tenant? This cannot be undone.")) {
                  deleteTenant(d._id);
            ...
            <div className="px-5 pb-5 border-t">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                ...
                <div>
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${d.status === "active" ? "bg-green-100 text-green-700" : d.status === "trial" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"}`}
                  >
                    {d.status}
                  </span>
                </div>
                <div>
                  ...
                  <p className="font-medium">
                    {d.planStartedAt
                      ? new Date(d.planStartedAt).toLocaleDateString()
                      : "-"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium">
                    {d.planExpiresAt
                      ? new Date(d.planExpiresAt).toLocaleDateString()
                      : "-"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium">
                    {d.trialEndsAt
                      ? new Date(d.trialEndsAt).toLocaleDateString()
                      : "-"}
                  </p>
                </div>
              </div>
            ...
            <div className="px-5 pb-5 border-t">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                {Object.entries(ALL_MODULE_LABELS).map(([key, label]) => {
                  const active = editingModules
                    ? selectedModules.includes(key)
                    : (s?.activeModules || []).includes(key);
                  return (
                    <button
                      key={key}
              ...
              {editingModules && (
                <p className="text-xs text-gray-500 mt-3">
                  Click modules to toggle. Changes take effect immediately for
                  tenant users on next page load.
                </p>
              )}
            </div>
          )}
        ...
        {s && (
          <>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="text-center px-5 py-2 font-medium text-gray-600">
                          Tickets
                        </th>
                        <th className="text-center px-5 py-2 font-medium text-gray-600">
                          Status
                    ...
                    <tbody className="divide-y">
                      {(s.agents || []).map((a: any) => (
                        <tr key={a._id} className="hover:bg-gray-50">
                          ...
                          <td className="px-5 py-2 text-center">
                            <span
                              className={`text-xs ${a.isActive ? "text-green-600" : "text-red-600"}`}
                            >
            ...
            <div className="bg-white rounded-xl border">
              ...
              >
                <h2 className="font-semibold">
                  Departments ({s.departments?.length || 0})
                </h2>
