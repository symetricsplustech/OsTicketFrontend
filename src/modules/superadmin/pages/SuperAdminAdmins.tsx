...
export default function SuperAdminAdmins() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateAdmin({ id: editingId, body: form });
      } else {
  ...
  const startEdit = (admin: any) => {
    ...
    setForm({
      ...
      email: admin.email,
      password: "",
      platformRole: admin.platformRole || "platform_administrator",
  ...
  const roleLabel = (r: string) =>
    PLATFORM_ROLES.find((pr) => pr.value === r)?.label || r;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage SaaS super admin accounts and platform roles
          </p>
        </div>
        <button
          onClick={() => {
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, platformRole: e.target.value })
                }
                className="w-full border rounded-lg px-3 py-2 text-sm"
              >
                ...
                <tr key={admin._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <span className="px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-700">
                      {roleLabel(admin.platformRole || "platform_owner")}
                    </span>
                  </td>
                  <td className="px-5 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${admin.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                    >
