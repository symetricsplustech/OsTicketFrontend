...
export default function SuperAdminPlans() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingId) {
      await updatePlan({ id: editingId, body: form });
    } else {
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, maxStorage: Number(e.target.value) })
                  }
                  className="w-full border rounded-lg px-3 py-2 text-sm"
                />
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          ...
          plans.map((p: any) => (
            <div
              key={p._id}
              className={`bg-white rounded-xl border p-5 ${p.isDefault ? "ring-2 ring-brand-300" : ""}`}
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1">
                  ...
                  <button
                    onClick={() => {
                      if (confirm("Delete this plan?")) deletePlan(p._id);
                    }}
