    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={counts.activeCompanies || 0}
          icon={Building2}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          label="MRR"
          value={`₹${Math.round(counts.mrr || 0).toLocaleString()}`}
          icon={TrendingUp}
          color="bg-emerald-100 text-emerald-600"
        />
        <StatCard
          label="ARR"
          value={`₹${Math.round(counts.arr || 0).toLocaleString()}`}
          icon={DollarSign}
          color="bg-green-100 text-green-600"
        />
        <StatCard
      ...
      <div className="rounded-xl border bg-white p-4 text-sm text-gray-600">
        <span className="mr-6">
          <strong>{counts.newToday || 0}</strong> tenants today
        </span>
        <span className="mr-6">
          <strong>{counts.newThisMonth || 0}</strong> this month
        </span>
        <span>
          <strong>{counts.failedInvoices || 0}</strong> failed payments
        </span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
              ...
              (tenants?.data || []).map((t: any) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${t.status === "active" ? "bg-green-100 text-green-700" : t.status === "trial" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"}`}
                    >
                      {t.status}
                    </span>
          ...
          <div className="flex items-end gap-3 h-40">
            {dashboard.data.companyDistribution.map((d: any) => {
              const maxCount = Math.max(
                ...dashboard.data.companyDistribution.map((x: any) => x.count),
                1,
              );
