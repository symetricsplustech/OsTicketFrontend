...
export function RecordTable({
  ...
  const allSelected =
    records.length > 0 && records.every((r: any) => selected.has(r._id));
  ...
  if (isLoading)
    return (
      <div className="p-6 text-gray-400 animate-pulse">Loading {entity}…</div>
    );
    ...
    <div className="space-y-3">
      ...
      {selected.size > 0 && (
        <div className="bg-brand-50 border border-brand-200 rounded-lg px-4 py-2 flex items-center gap-3 text-sm">
          ...
          <Button onClick={applyBulk} disabled={!bulkStatus} size="sm">
            Apply
          </Button>
          <Button
            ...
            {records.map((row: any) => (
              ...
              >
                ...
                {columns.map((col) => (
                  <td
                    key={col.key}
                    className="px-4 py-2.5 text-sm text-gray-600"
                  >
                    {col.render
...
export function RecordFormInline({
  entity,
  editable,
  refs,
  onDone,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await create({ entity, body: values }).unwrap();
      setValues({});
      onDone?.();
    } catch (err: any) {
      setError(err?.data?.error || "Creation failed");
    }
  };
            ...
            <div key={f}>
              ...
              {isRef && refEntity ? (
                <RefSelect
                  entity={refEntity}
                  value={values[f]}
                  onChange={(v) => setValues((v2) => ({ ...v2, [f]: v }))}
                />
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${colors[status] || "bg-gray-100 text-gray-600"}`}
    >
      {status}
    </span>
  );
