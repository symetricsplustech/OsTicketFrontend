...
export function ModuleDashboard({
  ...
  const stats = useMemo(() => {
    ...
    for (const r of records) {
      ...
      if (["closed", "resolved"].includes(r.status)) closed++;
      else open++;
    }
    ...
    <div className="space-y-6">
            ...
            <div key={status} className="flex items-center gap-3">
              ...
              <div className="flex-1 bg-gray-100 rounded-full h-5 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (count / Math.max(...Object.values(stats.byStatus), 1)) * 100)}%`,
                  }}
                >
      ...
      <div className="grid grid-cols-4 gap-4">
        {["critical", "high", "medium", "low"].map((p) => (
          <div
            key={p}
            className={`rounded-lg border p-4 ${p === "critical" ? "border-red-200 bg-red-50" : p === "high" ? "border-orange-200 bg-orange-50" : "bg-white"}`}
          >
