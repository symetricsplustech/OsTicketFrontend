        ...
        <div className="sticky top-0 bg-white border-b px-5 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <h2 className="text-lg font-bold text-gray-900 truncate">
              {record.title || record.name || record.number || id.slice(-8)}
            </h2>
          </div>
          <button
            onClick={onClose}
              ...
              {related.map((rel: any) => (
                <div key={rel.entity} className="mb-2">
                  ...
                  {rel.records.map((r: any) => (
                    <div
                      key={r._id}
                      className="text-sm text-gray-600 py-0.5 border-l-2 border-brand-200 pl-2"
                    >
...
function EditableFields({ entity, record, editable, update }: any) {
  ...
  useEffect(() => {
    if (record)
      setValues(
        Object.fromEntries(editable.map((f: string) => [f, record[f] ?? ""])),
      );
  }, [record]);
        ...
        <div className="space-y-3">
          {editable.map((f: string) => (
            <div key={f}>
              ...
              {["status", "priority", "severity", "category", "stage"].includes(
                f,
              ) ? (
                <select
                  value={values[f] ?? ""}
                ...
                <input
                  ...
                  checked={!!values[f]}
                  onChange={(e) =>
                    setValues((v) => ({ ...v, [f]: e.target.checked }))
                  }
                  className="rounded"
                />
              ) : (
