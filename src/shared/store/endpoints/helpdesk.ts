...
const helpdeskApi = platformApi.injectEndpoints({
  endpoints: (b) => ({
    ...
    >({
      query: ({ ticketNumber, ...body }) => ({
        url: `/gaps/worklogs/${ticketNumber}`,
        method: "POST",
        body,
      }),
    ...
    setPriorityMatrix: b.mutation<
      any,
      { cells: Array<{ impact: string; urgency: string; priority: string }> }
    >({
    ...
    computePriority: b.mutation<any, { impact: string; urgency: string }>({
      query: (body) => ({
        url: "/gaps3/priority-matrix/compute",
        method: "POST",
        body,
      }),
    }),
