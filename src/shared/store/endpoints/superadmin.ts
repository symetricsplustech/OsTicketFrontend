...
const superadminApi = platformApi.injectEndpoints({
  endpoints: (b) => ({
    ...
    setTenantStatus: b.mutation<any, { id: string; status: string }>({
      query: ({ id, ...body }) => ({
        url: `/superadmin/companies/${id}/status`,
        method: "PUT",
        body,
      }),
      invalidatesTags: ["SaTenant"],
    ...
    setTenantPlan: b.mutation<any, { id: string; planId: string }>({
      query: ({ id, ...body }) => ({
        url: `/superadmin/companies/${id}/plan`,
        method: "PUT",
        body,
      }),
      invalidatesTags: ["SaTenant"],
    ...
    updateTenantModules: b.mutation<any, { id: string; modules: string[] }>({
      query: ({ id, modules }) => ({
        url: `/superadmin/companies/${id}/modules`,
        method: "PUT",
        body: { modules },
      }),
      invalidatesTags: ["SaTenant"],
    }),
    impersonateTenant: b.mutation<
      any,
      { companyId: string; reason: string; ttlMinutes?: number }
    >({
      query: (body) => ({
    ...
    getSaOperationsJobs: b.query<any, { page?: number; limit?: number }>({
      query: (params) => ({
        url: "/superadmin/operations/jobs",
        params: params as any,
      }),
      providesTags: ["SaOperations"],
    }),
    ...
    forceLogout: b.mutation<any, { userId: string; reason?: string }>({
      query: (body) => ({
        url: "/superadmin/security/force-logout",
        method: "POST",
        body,
      }),
