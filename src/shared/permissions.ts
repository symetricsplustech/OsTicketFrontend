import {
  ITSM_PERMISSION_MODULES,
  ITSM_PERMISSION_KEYS,
} from "./itsmPermissions.generated";
...
export const ITSM_KEY_PREFIXES = ITSM_MODULES.map((m) =>
  m.namespace.replace("*", ""),
);

// Helper: test whether a permission key is a canonical ITSM key
export const isItsmKey = (
  key: string,
): key is `${string}.${string}.${string}` => {
...
const CANONICAL_BY_LEGACY = Object.entries(LEGACY_ALIAS_MAP).reduce<
  Record<string, string[]>
>((result, [canonical, legacyKeys]) => {
  legacyKeys.forEach((legacy) => {
    result[legacy] = [...(result[legacy] || []), canonical];
  });
  return result;
}, {});
 ...
 */
export const can = (key: string, userPermissions: string[] = []): boolean => {
  if (userPermissions.includes("*") || userPermissions.includes(key))
    return true;

  // Explicit namespace grants are supported at any segment boundary.
  if (
    userPermissions.some((grant) => {
  ...
  if (canonicalAliases.some((canonical) => userPermissions.includes(canonical)))
    return true;

  // 2. canonical itsm.* key – try legacy alias map
  if (isItsmKey(key)) {
    const aliases = LEGACY_ALIAS_MAP[key];
    if (aliases && aliases.some((leg) => userPermissions.includes(leg)))
      return true;
...
// ─── Module Keys ───────────────────────────────────────────────────────
export const MODULE_KEYS = ["helpdesk", "settings", "ai"] as const;
...
// Read/UI keys for the full HelpDesk console (all ITSM modules 1-12, incl.
// walk-up / on-call / major-incident). The owner can still restrict per user.
